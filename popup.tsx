...
function IndexPopup() {
	...
	const fetchSettings = async () => {
		const hideCircleTweets = await chrome.storage.local.get([
			"hideCircleTweets",
		]);

		let value = hideCircleTweets.hideCircleTweets;

		const isEmptyItem =
			Object.keys(hideCircleTweets).length === 0 &&
			hideCircleTweets.constructor === Object;

		if (isEmptyItem) {
			await chrome.storage.local.set({ hideCircleTweets: true });
			value = true;
		}
		...
		return [
			{
				value: value,
				label: "Hide Twitter Circle Tweets",
				key: "hideCircleTweets",
			},
		];
	};
